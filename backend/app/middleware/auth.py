...
class AuthMiddleware:
    ...
    def create_access_token(self, user_id: int, expires_delta: Optional[int] = None) -> str:
    ...
    def require_role(
        ...
        def role_checker(
            ...
            # Check specific role
            if not self.check_user_role(current_user, required_role, domain_id, class_id, db):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
    ...
    def require_instructor(self, domain_id: Optional[int] = None, class_id: Optional[int] = None):
